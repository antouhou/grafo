...
use crate::backend::metrics::PhaseTimings;
...
pub(in crate::backend) struct BgraReadbackResources {
    pub(in crate::backend) texture: Texture,
    view: TextureView,
    mapping: ReadbackMapping,
    pub(in crate::backend) buffer: Buffer,
}
...
pub(in crate::backend) struct ArgbReadbackTarget {
    pub(in crate::backend) texture: Texture,
    view: TextureView,
    mapping: ReadbackMapping,
...
pub(in crate::backend) struct ArgbReadbackResources {
    pipeline: ComputePipeline,
    bind_group_layout: BindGroupLayout,
    pub(in crate::backend) target: ArgbReadbackTarget,
}
...
impl WgpuBackend {
    ...
    pub(in crate::backend) fn render_to_buffer(
        &mut self,
        commands: &RenderPlan,
        buffer: &mut Vec<u8>,
    ) -> Result<(), ReadbackError> {
    ...
    pub(in crate::backend) fn render_to_argb32(
        &mut self,
        commands: &RenderPlan,
        out_pixels: &mut [u32],
    ) -> Result<(), ReadbackError> {
