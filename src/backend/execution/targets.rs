...
pub(in crate::backend) fn set_scissor(
...
pub(in crate::backend) struct RenderTarget<'a> {
...
impl<'a> RenderTarget<'a> {
    pub(in crate::backend) fn resolved_view(&self) -> &'a TextureView {
        self.resolve_target.unwrap_or(self.color_view)
    }

    pub(in crate::backend) fn for_output(
    ...
    pub(in crate::backend) fn for_texture(texture: &'a PooledTexture) -> Self {
        Self {
    ...
    pub(in crate::backend) fn clear_if_needed(&mut self, encoder: &mut CommandEncoder) {
        if self.needs_clear {
    ...
    pub(in crate::backend) fn begin_pass<'pass>(
        &mut self,
...
pub(in crate::backend) struct SurfaceTarget<'a> {
    pub(in crate::backend) output: RenderTarget<'a>,
    pub(in crate::backend) capture_texture: Option<&'a Texture>,
}
