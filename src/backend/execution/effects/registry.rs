...
use crate::backend::errors::EffectResourceError;
...
impl EffectRegistry {
    ...
    pub(crate) fn load(
        ...
        effect_id: u64,
        pass_sources: &[&str],
    ) -> Result<bool, EffectResourceError> {
    ...
    pub(crate) fn validate_params(
        ...
        let effect = self
            .loaded
            .get(&effect_id)
            .ok_or(EffectResourceError::EffectNotLoaded(effect_id))?;
        let expects_params = effect.params_bind_group_layout.is_some();
        if expects_params && params.is_empty() {
            return Err(EffectResourceError::InvalidParams(format!(
