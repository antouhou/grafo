...
use crate::backend::execution::textures::IntermediateTextureResources;
use crate::backend::execution::uniforms;
use crate::backend::gradient::{GpuGradientColorParams, GpuMaterialParams};
...
#[derive(Default)]
pub(in crate::backend) struct TextureMaterialPool {
    slots: Vec<TextureMaterialSlot>,
    used: usize,
}

impl TextureMaterialPool {
    pub(in crate::backend) fn begin_render(&mut self) {
        self.used = 0;
    }
    ...
    pub(in crate::backend) fn finish_render(&mut self) {
        self.slots.truncate(self.used);
    }

    pub(in crate::backend) fn invalidate_bindings(&mut self) {
...
impl ShapeDrawResources {
    ...
    pub(in crate::backend) fn material_bind_group(
        &self,
...
impl ShapeExecutionResources {
    #[allow(clippy::too_many_arguments)]
    pub(in crate::backend) fn prepare_texture_material(
        &mut self,
        encoder: &mut CommandEncoder,
        draw: ShapeDraw,
        device: &Device,
