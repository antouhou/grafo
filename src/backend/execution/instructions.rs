...
use crate::backend::resources::{Buffers, RendererPipelineResources};
...
pub(in crate::backend) struct ExecutionContext<'a> {
    pub device: &'a Device,
    pub queue: &'a Queue,
...
pub(in crate::backend) struct ExecutionResources<'a> {
...
pub(in crate::backend) struct ExecutionMetrics {
    #[cfg(feature = "render_metrics")]
...
pub(in crate::backend) fn execute_commands(
    encoder: &mut CommandEncoder,
    commands: &RenderPlan,
    surface: SurfaceTarget<'_>,
