...
pub(in crate::backend) struct ShapeEffectRendererResources {
    pub mask_bind_group_layout: BindGroupLayout,
    pub mask_pipeline: RenderPipeline,
}

impl ShapeEffectRendererResources {
    pub(in crate::backend) fn new(device: &Device, format: TextureFormat) -> Self {
    ...
    pub(in crate::backend) fn recreate_pipeline(&mut self, device: &Device, format: TextureFormat) {
