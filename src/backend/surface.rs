...
use crate::backend::pipeline::{create_and_depth_texture, create_msaa_color_texture};
...
impl WgpuBackend {
    pub(in crate::backend) fn resize(
        &mut self,
    ...
    pub(in crate::backend) fn normalize_msaa_sample_count(requested: u32) -> u32 {
        match requested {
            0 | 1 => 1,
            2..=4 => 4,
            _ => {
    ...
    pub(in crate::backend) fn recreate_msaa_texture(&mut self) {
    ...
    pub(in crate::backend) fn recreate_depth_stencil_texture(&mut self) {
    ...
    pub(in crate::backend) fn configure_surface(&self, surface: &Option<Surface<'_>>) {
        if let Some(surface) = surface {
            surface.configure(&self.device, &self.config);
        }
    }

    pub(in crate::backend) fn set_vsync(
        &mut self,
        surface: &mut Option<wgpu::Surface<'_>>,
        vsync: bool,
    ) {
