...
pub(crate) fn backdrop_layer_params(
    ...
    // WGPU's texture dimension limit is below i32::MAX.
    [
        capture_origin.0,
        capture_origin.1,
        i32::try_from(source_size.0).expect("backdrop source width must fit in i32"),
        i32::try_from(source_size.1).expect("backdrop source height must fit in i32"),
    ]
}

/// Creates the layout for the input texture and sampler at group 0.
pub(crate) fn create_effect_input_bind_group_layout(device: &Device) -> BindGroupLayout {
...
pub(crate) fn create_effect_params_bind_group_layout(device: &Device) -> BindGroupLayout {
    device.create_bind_group_layout(&BindGroupLayoutDescriptor {
