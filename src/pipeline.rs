...
use crate::vertex::{
    CustomVertex, InstanceColor, InstanceMetadata, InstanceOcclusion, InstanceTransform,
};
...
/// Creates a bind group layout for the occlusion rects storage buffer at @group(3).
pub fn create_occlusion_rects_bind_group_layout(device: &Device) -> BindGroupLayout {
    device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("occlusion_rects_bind_group_layout"),
        entries: &[wgpu::BindGroupLayoutEntry {
            binding: 0,
...
pub fn create_pipeline(
    ...
    let occlusion_rects_bgl = create_occlusion_rects_bind_group_layout(device);
...
pub fn create_stencil_only_pipeline(
    ...
    occlusion_rects_bgl: &wgpu::BindGroupLayout,
) -> RenderPipeline {
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
