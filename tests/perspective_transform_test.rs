...
use grafo::{InstanceRenderParams, TransformInstance};
...
fn perspective_matrix(d: f32) -> Transform3D<f32, (), ()> {
    ...
    Transform3D::new(
        ...
        0.0,
        1.0,
        1.0 / d, // row 3: m34 = +1/d for CSS-style perspective
        0.0,
        0.0,
        0.0,
        1.0, // row 4
...
mod tests {
    ...
    fn test_star_wars_tilt_transform() {
        ...
        let tl_transformed =
            apply_transform_with_perspective(top_left, transform, perspective_distance, None);
        let tr_transformed =
            apply_transform_with_perspective(top_right, transform, perspective_distance, None);
        let bl_transformed =
            apply_transform_with_perspective(bottom_left, transform, perspective_distance, None);
        let br_transformed =
            apply_transform_with_perspective(bottom_right, transform, perspective_distance, None);
        ...
        println!(
            "Top-left: ({:.3}, {:.3})",
            tl_transformed[0], tl_transformed[1]
        );
        println!(
            "Top-right: ({:.3}, {:.3})",
            tr_transformed[0], tr_transformed[1]
        );
        println!(
            "Bottom-left: ({:.3}, {:.3})",
            bl_transformed[0], bl_transformed[1]
        );
        println!(
            "Bottom-right: ({:.3}, {:.3})",
            br_transformed[0], br_transformed[1]
        );
    }
    ...
    fn test_translation_preserves_perspective_ratio() {
        ...
        let rect1_corners = [[-50.0, -50.0], [50.0, -50.0], [-50.0, 50.0], [50.0, 50.0]];

        // Apply rotation and perspective to rectangle 1 (no offset)
        let transformed1: Vec<[f32; 2]> = rect1_corners
            .iter()
            .map(|&corner| {
                apply_transform_with_perspective(corner, rotation, perspective_distance, None)
            })
            .collect();
        ...
        let transformed1_translated: Vec<[f32; 2]> = rect1_corners
            .iter()
            .map(|&corner| {
                apply_transform_with_perspective(
                    corner,
                    rotation,
        ...
        assert!(
            ...
            "X translation should be {}",
            translation_x
        );
        assert!(
            (transformed1_translated[0][1] - transformed1[0][1] - translation_y).abs() < epsilon,
            "Y translation should be {}",
            translation_y
        );

        println!("\nOriginal perspective-transformed rectangle:");
        println!(
            "  Top width: {:.3}, Bottom width: {:.3}",
            width_top_1, width_bottom_1
        );
        println!("  Left height: {:.3}", height_left_1);
        println!(
            "  Top-left corner: ({:.3}, {:.3})",
            transformed1[0][0], transformed1[0][1]
        );

        println!(
            "\nAfter 2D translation (+{}, +{}):",
            translation_x, translation_y
        );
        println!(
            "  Top width: {:.3}, Bottom width: {:.3}",
            width_top_1_trans, width_bottom_1_trans
        );
        println!("  Left height: {:.3}", height_left_1_trans);
        println!(
            "  Top-left corner: ({:.3}, {:.3})",
            transformed1_translated[0][0], transformed1_translated[0][1]
        );
    }
    ...
    fn test_shader_vertex_transform_basic() {
        ...
        println!(
            "  Top-left (0, 0) -> NDC ({:.3}, {:.3})",
            ndc_tl[0], ndc_tl[1]
        );
        println!(
            "  Bottom-right (800, 600) -> NDC ({:.3}, {:.3})",
            ndc_br[0], ndc_br[1]
        );
    }
    ...
    fn test_shader_vertex_transform_with_offset() {
        ...
        println!(
            "  Position (0, 0) + viewport_position (400, 300) -> NDC ({:.3}, {:.3})",
            ndc[0], ndc[1]
        );
    }
    ...
    fn test_shader_vertex_transform_with_perspective() {
        ...
        let corners = [[-50.0, -50.0], [50.0, -50.0], [-50.0, 50.0], [50.0, 50.0]];
        ...
        let ndc_corners: Vec<[f32; 2]> = corners
            .iter()
            .map(|&corner| {
                shader_vertex_transform(corner, &rotation_transform, &render_params, canvas_size)
            })
            .collect();
        ...
        println!(
            "  Top-left: ({:.3}, {:.3})",
            ndc_corners[0][0], ndc_corners[0][1]
        );
        println!(
            "  Top-right: ({:.3}, {:.3})",
            ndc_corners[1][0], ndc_corners[1][1]
        );
        println!(
            "  Bottom-left: ({:.3}, {:.3})",
            ndc_corners[2][0], ndc_corners[2][1]
        );
        println!(
            "  Bottom-right: ({:.3}, {:.3})",
            ndc_corners[3][0], ndc_corners[3][1]
        );
        println!(
            "  Top width (NDC): {:.3}, Bottom width (NDC): {:.3}",
            width_top_ndc, width_bottom_ndc
        );
        println!(
            "  Width ratio: {:.3} (expected: {:.3})",
    ...
    fn test_shader_vertex_transform_matches_manual() {
        ...
        for &corner in &corners {
            ...
            let ndc_shader =
                shader_vertex_transform(corner, &transform, &render_params, canvas_size);
    ...
    fn test_shader_vertex_transform_offset_preserves_ratio() {
        ...
        let ndc_no_offset: Vec<[f32; 2]> = corners
            .iter()
            .map(|&corner| {
                shader_vertex_transform(corner, &transform, &render_params_no_offset, canvas_size)
            })
            .collect();
        ...
        let ndc_with_offset: Vec<[f32; 2]> = corners
            .iter()
            .map(|&corner| {
                shader_vertex_transform(corner, &transform, &render_params_with_offset, canvas_size)
            })
            .collect();
