...
impl Default for App {
    fn default() -> Self { Self { renderer: None, bg_tex_id: 100, fg_tex_id: 101, shape_id: None } }
}
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let mut renderer = futures::executor::block_on(Renderer::new(window, physical_size, scale_factor, true, false));

        // Create a simple rectangle shape
        let shape_id = renderer.add_shape(
            Shape::rect([(100.0, 100.0), (500.0, 400.0)], Color::rgb(200, 200, 200), Stroke::new(1.0, Color::BLACK)),
            None,
            None,
        );
        ...
        for y in 0..h { for x in 0..w { let idx = ((y * w + x) * 4) as usize; let checker = ((x / 32) + (y / 32)) % 2 == 0; let c = if checker { 60 } else { 180 }; bg[idx] = c; bg[idx+1] = c; bg[idx+2] = c; bg[idx+3] = 255; }}
        ...
        let cx = w as f32 / 2.0; let cy = h as f32 / 2.0; let r = w.min(h) as f32 * 0.4;
        for y in 0..h { for x in 0..w { let idx = ((y * w + x) * 4) as usize; let dx = x as f32 - cx; let dy = y as f32 - cy; let d = (dx*dx + dy*dy).sqrt(); if d <= r { let a = ((r - d) / r).clamp(0.0,1.0); let alpha = (a * 255.0) as u8; fg[idx] = 255; fg[idx+1] = 255; fg[idx+2] = 255; fg[idx+3] = alpha; }} }
        ...
        if let Some(r) = self.renderer.as_mut() { let _ = r.render(); r.clear_draw_queue(); }
    }
